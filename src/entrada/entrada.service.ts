...
import { DisponibilidadButacaService } from '../servicios/disponibilidadButaca.service';
...
export class EntradaService {
    ...
    async nuevaEntrada (datos:EntradaInput): Promise<EntradaResponse> {
          if (datos.DisponibilidadButacaId) {
              const esValida = await this.disponibilidadButaca.validarDisponibilidad(datos.DisponibilidadButacaId);
              if (!esValida) throw new BadRequestException(`Promoción con id ${datos.DisponibilidadButacaId} no válida`);
            }
...
async updateEntrada(id: number, datos: EntradaInput): Promise<EntradaResponse> {
